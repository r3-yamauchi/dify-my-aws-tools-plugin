"""
場所: tools/opensearch_knn_search.py
内容: Bedrock の埋め込みモデルでクエリをベクトル化し、Amazon OpenSearch の kNN 検索を実行するツール。
目的: テキスト/画像の類似検索結果をメタデータ付きで取得し、RAG パイプラインに組み込めるようにする。
"""
...
class OpenSearchRetrieveTool(Tool):
    ...
    def _invoke(
        ...
        """OpenSearch への接続・埋め込み生成・kNN 検索を順に実行する."""
        try:
            ...
            reset_clients_on_credential_change(
                self,
                credentials,
                ["os_client", "s3_client", "bedrock_client"],
            )
            ...
            if not self.os_client:
                ...
                aws_auth_credentials = session.get_credentials()
                awsauth = AWSV4SignerAuth(aws_auth_credentials, aws_region, "aoss")

                # OpenSearch クライアントを初期化（署名付き接続）
            ...
            if not self.s3_client:
                self.s3_client = boto3.client(service_name="s3", **client_kwargs)

            if not self.bedrock_client:
                self.bedrock_client = boto3.client(service_name="bedrock-runtime", **client_kwargs)
