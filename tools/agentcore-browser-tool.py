"""
場所: tools/agentcore-browser-tool.py
内容: AgentCore Browser セッションと Playwright を利用して、URL 閲覧・検索・フォーム入力・JS 実行などを行うツール。
目的: Session Manager で生成した WebSocket 情報を Parameter Store から取得し、Dify からリモートブラウザ操作を可能にする。
"""
...
from provider.utils import ParameterStoreManager, resolve_aws_credentials

class AgentcoreBrowserToolTool(Tool):
    _sessions = {}  # browser_session_id ごとの Playwright リソースをキャッシュ
    ...
    def _setup_temp_dir(self):
        """Playwright の一時ディレクトリをユーザー別に用意する."""
        try:
            # 创建用户专用的临时目录
    ...
    async def _init_browser_session(self, browser_session_id:str, aws_region:str, credentials: dict[str, Optional[str]]) -> dict:
        """Parameter Store の情報を使ってブラウザーセッションへ接続する."""
        try:
    ...
    async def _cleanup_browser(self, session_id: str = None):
        """Playwright リソースとブラウザーをクローズし、キャッシュを削除する."""
        try:
            if not session_id:
                return
    ...
    async def _browse_url(self, browser_session_id:str, url: str, wait_time: int = 3) -> dict:
        """既存ブラウザーセッションで任意の URL を開き、本文を取得する."""
        try:
            session = self._get_session(browser_session_id)
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            asyncio.run(self._init_browser_session(browser_session_id, aws_region, credentials))
