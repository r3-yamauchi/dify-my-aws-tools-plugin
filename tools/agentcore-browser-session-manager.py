"""
場所: tools/agentcore-browser-session-manager.py
内容: Bedrock AgentCore BrowserClient を用いてブラウザーセッションの生成/終了を管理するツール。
目的: Dify からリモートブラウザーの WebSocket 情報を Systems Manager Parameter Store に保存し、他ツールが参照できるようにする。
"""
...
class AgentcoreBrowserSessionManagerTool(Tool):
    ...
    def _init_browser_session(self, aws_region, session_timeout_seconds, credentials: dict[str, Optional[str]]) -> dict:
        """新しい AgentCore Browser セッションを起動し、接続情報を SSM に保存する."""
    ...
    def _close_browser_session(self, aws_region, session_id, credentials: dict[str, Optional[str]]) -> dict:
        """既存のブラウザーセッションを停止し、SSM の情報も削除する."""
        browser_client = BrowserClient(aws_region)
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        if function_name == "init_browser_session":
            result = self._init_browser_session(aws_region, session_timeout_seconds, credentials)
            ...
            result = self._close_browser_session(aws_region, session_id, credentials)
